...
exports.arrayValidation = (data) => {
  const errors = {};
  for (let x of data) {
    errors[x._id] = {};
    const result = this.registerValidationCustom(x, true);
    if (result) errors[x._id] = result;
    if (x._id)
      if (!this.objectIdvalidation(x._id))
        errors[x._id].id = { message: "Invalid id." };
    if (_.isEmpty(errors[x._id])) {
      delete errors[x._id];
    }
  }
